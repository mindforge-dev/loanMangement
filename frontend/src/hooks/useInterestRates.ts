...
export const interestRateKeys = {
  ...
  list: (filters: string) =>
    [...interestRateKeys.lists(), { filters }] as const,
...
export const useInterestRate = (
  id: string,
): UseQueryResult<InterestRate, Error> => {
  return useQuery({
...
export const useCreateInterestRate = (): UseMutationResult<
  InterestRate,
  Error,
  CreateInterestRateDto
> => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: interestRateKeys.detail(data.id),
      });
    },
  });
};

export const useDeleteInterestRate = (): UseMutationResult<
  void,
  Error,
  string
> => {
