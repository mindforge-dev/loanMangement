...
export default function TransactionFormModal({
    ...
    const { data: loansResponse, isLoading: loansLoading } = useLoans({ page: 1, limit: 1000 })
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.remaining_balance || Number.isNaN(remainingBalance) || remainingBalance < 0) {
            newErrors.remaining_balance = 'Remaining balance must be a valid number (>= 0)'
        }

        const termMonths = Number(formData.payment_term_months)
        if (!formData.payment_term_months || !Number.isInteger(termMonths) || termMonths < 1) {
            newErrors.payment_term_months = 'Payment term must be a whole number (>= 1)'
        }
                    ...
                    <form onSubmit={handleSubmit} className="px-8 py-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                {errors.borrower_id && <p className="mt-1 text-sm text-red-500">{errors.borrower_id}</p>}
                            </div>

                            <div>
                                <label htmlFor="loan_id" className="block text-sm font-semibold text-gray-700 mb-2">
                                ...
                                {errors.loan_id && <p className="mt-1 text-sm text-red-500">{errors.loan_id}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    id="type"
                                    value={formData.type}
                                    onChange={(e) => updateField('type', e.target.value as CreateTransactionDto['type'])}
                            ...
                            <div>
                                <label htmlFor="amount_paid" className="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            <div>
                                <label htmlFor="remaining_balance" className="block text-sm font-semibold text-gray-700 mb-2">
                                ...
                                />
                                {errors.remaining_balance && <p className="mt-1 text-sm text-red-500">{errors.remaining_balance}</p>}
                            </div>

                            <div>
                                <label htmlFor="payment_term_months" className="block text-sm font-semibold text-gray-700 mb-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateField('payment_term_months', e.target.value)}
                                    placeholder="12"
                                ...
                                />
                                {errors.payment_term_months && <p className="mt-1 text-sm text-red-500">{errors.payment_term_months}</p>}
                            </div>
